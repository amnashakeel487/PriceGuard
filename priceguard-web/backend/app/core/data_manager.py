...
PRODUCTS_FIELDNAMES = [
    ...
    "highest_price", "lowest_price", "last_checked", "alert_sent", "user_email",
]
    ...
    def load_products(self) -> List[Product]:
        try:
            with open(self.products_file, "r", newline="", encoding="utf-8") as f:
                ...
                products = [Product.from_dict(row) for row in reader if row.get("url")]

                # Auto-heal product names if saved as generic domain titles like "Amazon.com"
                from .scraper import Scraper
                modified = False
                for p in products:
                    cleaned_name = Scraper._clean_product_name(p.name)
                    if not cleaned_name:
                        slug_name = Scraper._extract_name_from_url(p.url)
                        if slug_name:
                            p.name = slug_name
                            modified = True
                if modified:
