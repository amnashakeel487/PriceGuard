// In dev: Vite proxy rewrites /api → localhost:8000 so BASE_URL stays ""
...
export const api = {
  auth: {
    ...
    verify: (email, otp) =>
      request("/api/auth/verify", { method: "POST", body: JSON.stringify({ email, otp }) }),
    resendOtp: (email) =>
      request("/api/auth/resend-otp", { method: "POST", body: JSON.stringify({ email }) }),
  ...
  updateProduct: (url, target_price) =>
    request(`/api/products?url=${encodeURIComponent(url)}`, { method: "PATCH", body: JSON.stringify({ target_price }) }),
